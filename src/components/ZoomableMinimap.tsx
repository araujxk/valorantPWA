...
export default function ZoomableMinimap({
  src, mapName, pin, angle: externalAngle, onPinPlace, onAngleChange, className = "", abilityId
  ...
  return (
    ...
    >
      ...
      {onPinPlace && scale <= 1.05 && !isDirectional && <div className="absolute bottom-2 left-2 z-10 text-[9px] text-slate-500 bg-slate-900/70 px-2 py-0.5 rounded-lg pointer-events-none">scroll p/ zoom · arrasta p/ mover</div>}
      {onPinPlace && isDirectional && <div className="absolute bottom-2 left-2 z-10 text-[9px] text-slate-400 bg-slate-900/80 px-2 py-0.5 rounded-lg pointer-events-none border border-slate-700/50">📍 click: posição · ⟳ handle: rodar</div>}
